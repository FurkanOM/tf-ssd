...
class CustomLoss(Layer):
    ...
    def __init__(self, neg_pos_ratio, loc_loss_weight, **kwargs):
    ...
    def get_config(self):
        ...
        config.update({"neg_pos_ratio": self.neg_pos_ratio, "loc_loss_weight": self.loc_loss_weight})
        return config
    ...
    def call(self, inputs):
        ...
        neg_bbox_indices_count = tf.math.count_nonzero(pos_cond, axis=1) * int(self.neg_pos_ratio)
        ...
        #
        return loc_loss * self.loc_loss_weight, conf_loss
...
def get_model(hyper_params, loc_loss_weight=10, mode="training"):
    """Generating ssd model for hyper params.
    inputs:
        hyper_params = dictionary
        loc_loss_weight = localization loss multiplier, to prevent the imbalanced loss calculation
    ...
    if mode == "training":
        ...
        #
        loc_loss, conf_loss = CustomLoss(hyper_params["neg_pos_ratio"], loc_loss_weight, name="custom_loss_calculation")(
        ...
        ssd_model.add_metric(loc_loss, name="ssd_loc_loss", aggregation="mean")
        ssd_model.add_loss(conf_loss)
        ssd_model.add_metric(conf_loss, name="ssd_conf_loss", aggregation="mean")
    else:
